...
<tasks>
- Add shared DTOs to `@aura/contract` for the MCP-facing families that are currently missing in TS: explainability, cross-namespace digest, correction/review queues, suggested corrections, namespace governance, memory health, and any shared maintenance-history structs that are reused across packages.
