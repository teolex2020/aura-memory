...
<tasks>
- Add shared DTOs to `@aura/contract` for the MCP-facing families that are currently missing in TS. At minimum enumerate and place the following families explicitly so later plans do not rediscover them ad hoc:
  - explainability: `RecallExplanationItem`, provenance/connection evidence, explainability bundle members
  - analytics/governance: `CrossNamespaceDigest`, digest options, compact summary, `NamespaceGovernanceStatus`, `MemoryHealthDigest`
  - correction/operator review: correction log entry, correction review queue candidate, contradiction review queue candidate, suggested correction/report
  - maintenance artifacts: `MaintenanceTrendSnapshot`, `ReflectionSummary`, and any shared issue/summary structs reused by core and storage
...
- If Rust research shows correction log or other phase-critical MCP read models persist on disk in a specific location, add the matching TS storage/helper layer here as well; do not guess the file contract.
