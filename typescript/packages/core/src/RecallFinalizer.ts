...
export function createRecallSessionTracker(): RecallSessionTracker {
  return Graph.createSessionTracker()
}
...
export function endRecallSession(
  ...
  return Effect.gen(function* () {
    ...
    if (result.updatedRecords.length > 0) {
      ...
      for (const record of result.updatedRecords) {
...
export function finalizeRecallRecords(
  ...
  return Effect.gen(function* () {
    ...
    yield* trackRecallSession(sessionTracker, sessionId, topIds)
    ...
    Effect.gen(function* () {
      ...
      return {
        finalize: (scored: RecallScored, sessionId?: string) => {
          const finalized = Effect.gen(function* () {
            const program = finalizeRecallRecords(brainDir, scored, sessionId, sessionTracker).pipe(
              Effect.provideService(FileRead, fileRead),
              Effect.provideService(FileWrite, fileWrite),
              Effect.provideService(Clock, clock)
            ) as Effect.Effect<void, FileReadError | FileWriteError | FileFormatError, never>
            yield* program
          })
          return finalized.pipe(Effect.mapError((cause) => new FinalizeError({ cause })))
        },
      }
    })
  )
