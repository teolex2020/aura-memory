...
- Build `explain_record`, `explain_recall`, and `explainability_bundle` on top of the now-complete maintenance/runtime data and the new evidence bridge rather than returning shrunken success payloads. If some fields remain empty because no correction events have occurred yet, they must stay present in the DTO shape as empty arrays/counts rather than being omitted.
