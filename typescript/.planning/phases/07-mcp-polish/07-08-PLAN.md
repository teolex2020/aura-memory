...
- Define the Rust MCP binary contract inside the harness/docs:
  - a pre-flight check that detects `cargo`, attempts `cargo build --bin aura-mcp` when the binary is absent, and records the exact binary path expected on Windows workspaces
