...
- Implement `namespace_governance_status` and `memory_health` using the persisted maintenance outputs added earlier plus instability/correction/policy data, but do not require any write path from `07-05`; this plan must stand on already-persisted or already-derived read models.
