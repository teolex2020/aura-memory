...
import { ExtractionError, ExtractionResult, FileRecord, Language } from "../types";
import { detectLanguage, initGrammars, isLanguageSupported, isSourceFile, loadGrammarsForLanguages } from "./grammars";
import { extractFromSource } from "./tree-sitter";
import { validatePathWithinRoot } from "@aura/utils";
 ...
 * (See issue #193.)
 */
export function collectGitFiles(
  repoDir: string,
  prefix: string,
  files: Set<string>,
): void {
...
export function scanDirectory(
  ...
  return scanDirectoryWalk(rootDir, onProgress, logDebug);
}
...
function scanDirectoryWalk(
  ...
  function walk(dir: string, matchers: ScopedIgnore[]): void {
    ...
    try {
      ...
      logDebug?.("Skipping unreadable directory", {
        dir,
        error: String(error),
      });
      return;
